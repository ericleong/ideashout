...
class ListCalendar(HTMLCalendar):
    ...
    def get_link(self, dt):
        ...
        """
        if Post.objects.filter(start_time__year=dt.year, start_time__month=dt.month, start_time__day=dt.day).exists():
            return reverse('events-day', kwargs={'year': dt.year, 'month': '%02d' % dt.month, 'day': '%02d' % dt.day})
        return False
